...
| 출처 표시 | 모든 답변에 페이지 번호 + 원문 미리보기 + 청크 ID 제공 |
...
> Faithfulness / Answer Relevancy는 배포 후 Solar 기준으로 재측정 예정
...
FastAPI /chat
    │
    ├─ 질문 임베딩 (jhgan/ko-sroberta-multitask)
    │
    ├─ ChromaDB 유사도 검색 (TOP_K=3)
    │
    ├─ 유사도 < 0.4 → "IPCC 보고서에서 찾을 수 없습니다" 반환
    │
    ├─ Solar LLM 답변 생성
    │
    └─ 답변 + 출처(page · preview · chunk_id) 반환
```
...
| IEP-2001 | 하이브리드 검색 (BM25 + Vector) | ⏳ 예정 |
...
Phase 3  검색 개선 (IEP-2001~1004)    ⏳ 배포 후 병행
