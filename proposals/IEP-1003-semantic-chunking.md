...
**3종 비교: IEP-1001 CASE 3 vs IEP-1002 vs IEP-1003**

| 유형 | IEP-1001 CASE3 | IEP-1002 | IEP-1003 | vs 1001 | vs 1002 |
| :--- | :---: | :---: | :---: | :---: | :---: |
...
> **IEP-1001 CASE 3을 최종 배포 청킹으로 채택한 이유**
>
> IEP-1003의 Recall 0.8611은 수치상 baseline(0.8520)을 소폭 상회하지만,
> 유효 샘플이 15개(NaN 85개 제외)에 불과해 **생존 편향**이 강하게 작용한 결과다.
> IEP-1001 CASE 3은 NaN 2.5개 기준으로 97개 이상의 유효 샘플에서 0.8520을 기록했다.
> 통계적 신뢰도 측면에서 IEP-1001 CASE 3이 훨씬 안정적인 선택이다.
...
IEP-1003의 표면적 수치(Recall 0.8611, Precision 0.5544)는 IEP-1001·1002 대비 우수해 보인다. 그러나 이 수치는 100개 중 NaN이 발생하지 않은 15개(Recall 기준) 샘플만의 평균이다. NaN 샘플 대부분은 평가 자체가 실패한 케이스로, 만약 이들이 낮은 점수로 측정되었다면 전체 평균은 크게 낮아질 것이다. 따라서 IEP-1003의 수치는 **"측정 가능했던 케이스에서의 성능"**으로 한정하여 해석해야 한다.

#### 생존 편향 보정: NaN을 0점으로 처리한 실제 성능 추정

낙관적 해석(성공 케이스만의 평균)과 보수적 추정(NaN을 0점으로 처리한 전체 모집합 기준)을 병기하면 시스템의 실제 신뢰도를 더 정직하게 파악할 수 있다.

| 지표 | 성공 케이스 평균 (낙관적) | 유효 샘플 수 | 전체 모집합 가중 평균 (보수적) | 비고 |
| :--- | :---: | :---: | :---: | :--- |
| Context Recall | 0.8611 | 15개 | **0.1292** | NaN 85개 × 0점 처리 |
| Context Precision | 0.5544 | 49개 | **0.2716** | NaN 51개 × 0점 처리 |
| Faithfulness | 0.3869 | 14개 | **0.0541** | NaN 86개 × 0점 처리 |
| Answer Relevancy | 0.5693 | 88개 | **0.5010** | NaN 12개 × 0점 처리 |

> **해석 지침**
>
> - 낙관적 수치는 "청킹 방식 자체의 잠재적 성능"을 나타낸다. 평가 인프라(judge LLM, 컨텍스트 크기)가 개선되면 이 수치에 근접할 수 있다.
> - 보수적 수치는 "현재 파이프라인 전체의 실제 안정성"을 나타낸다. 배포 가능 여부를 판단하는 기준으로 사용한다.
> - IEP-1001 CASE 3는 NaN 2.5개 기준으로 97개 이상의 유효 샘플에서 Recall 0.8520을 기록했으므로, 보수적 추정치도 **0.8307** 수준으로 유지된다. 이것이 IEP-1001 CASE 3을 배포용으로 채택한 핵심 근거다.

#### NaN 원인 분류 및 향후 검증 계획

NaN 발생 원인은 현재 두 가지 가설이 경합하고 있으며, 이를 구분하는 것이 Phase 3 실험 설계의 출발점이다.
...
두 가설은 상호 배타적이지 않으며, 실제로는 복합 원인일 가능성이 높다. **가설 B가 주요 원인이라면** Docling 교체 후 SemanticChunker의 NaN이 크게 감소하며 낙관적 수치(Recall 0.8611)에 통계적 신뢰도가 부여될 것이다. 이 경우 배포용 청킹을 SemanticChunker로 전환하는 근거가 된다.

### 의미 기반 청킹의 가능성과 한계

SemanticChunker는 IEP-1002의 헤딩 탐지 실패라는 제약을 우회하는 데는 성공했다. 비교(1.0000)와 범위 밖(1.0000) 유형에서 완벽한 Recall을 보인 것은 의미 단위 분리가 멀티 청크 검색에 유리하게 작용한 결과다. 그러나 pdfplumber 추출 텍스트의 노이즈가 SemanticChunker의 경계 결정에 직접적인 영향을 미쳐 청크 품질이 불균일해졌다. 이 문제는 Phase 3 IEP-1004(Docling 파서 교체)에서 근본적으로 해소할 수 있을 것으로 기대한다.

### 배포용 청킹 — IEP-1001 CASE 3 최종 채택

수치상 기준(Recall > 0.8520)은 충족했지만 NaN 85개라는 심각한 불안정성을 감안하여 **IEP-1001 CASE 3(1000/200 단순 청킹, Recall 0.8520, NaN 2.5개)**을 배포용 청킹으로 최종 채택한다. 안정성과 재현성 측면에서 단순 청킹이 현시점에서 가장 신뢰할 수 있는 선택이다. IEP-1003의 의미 기반 청킹은 Docling 파서 교체(IEP-1004) 이후 재평가한다.
...
- **Phase 2 즉시 진입**: IEP-1001 CASE 3 기반으로 FastAPI 서비스화(IEP-4001) 진행
